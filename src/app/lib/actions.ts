...
import { convertImageToRawBinary } from './utils';
...
export async function createItem(prevState: ItemState, formData: FormData) {
	...
	const imageBinary = await convertImageToRawBinary(imageName);
	...
	try {
      ...
      VALUES (${seller_id}, ${category_id}, ${price}, ${description}, ${title}, ${created}, ${imageBinary})
    `;
	} catch (error) {
...
export async function createSeller(prevState: SellerState, formData: FormData) {
	...
	const imageBinary = await convertImageToRawBinary(profilePicFile);
	...
	await sql`
		...
		VALUES (${first_name}, ${last_name}, ${description}, ${location}, ${email}, ${hashedPassword}, ${created}, ${created}, ${imageBinary})
	`;
